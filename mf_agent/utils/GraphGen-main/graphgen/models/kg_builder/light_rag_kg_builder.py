...
class LightRAGKGBuilder(BaseKGBuilder):
    ...
    async def extract(
        ...
        # save_extraction_to_json(chunk_id, content, final_result, "/data1/nuist_llm/mf-agent/GraphGen-main/build_data/data/step1_data/atomic/setp1_data.json")
    ...
    async def extract_knowledge_chunk(
        self, chunk: Chunk
    ):
        """
        Extract entities and relationships from a single chunk using the LLM client.
    ...
    async def merge_nodes(
        ...
        node = await kg_instance.get_node(entity_name) # 唯一的实体名称
        # 如果实体名称已经在Graph中
        if node is not None:
