...
TEMPLATE_ZH1: str = """你是一个NLP专家，擅长分析文本提取命名实体和关系。
...
文本：
################
由于我国再保险行业发展较晚,发展不充分,又经历了长期法定分布状态,造成国内再保险主体较少,目 前中资公司只有中再集团一家,还有7家海外再保险公司在国内开设分公司,2013年全国再保险保费为 1238亿元。国务院下发《关于加快发展现代保险服务业的若干意见》(简称"新国条"),新国条明确提出 加快发展再保险市场,增加再保险市场主体,发展区域性再保险中心。加大再保险产品和技术创新力度, 加大再保险对农业交通能源化工水利地铁航空航天核电及其他国家重点项目的大型风险 特殊风险的保险保障力度,增强再保险分散自然灾害风险的能力。强化再保险对我国海外企业的支持保障 功能,提升我国在全球再保险市场的定价权话语权。
################
输出：
("entity"{tuple_delimiter}"中再集团"{tuple_delimiter}"ORGANIZATION"{tuple_delimiter}"中国唯一的再保险中资公司，在再保险市场中占主导地位，是国内再保险主体的重要组成部分。"){record_delimiter}
("entity"{tuple_delimiter}"国务院"{tuple_delimiter}"ORGANIZATION"{tuple_delimiter}"中国政府最高行政机关，发布了《关于加快发展现代保险服务业的若干意见》政策，以促进再保险市场发展。"){record_delimiter}
("entity"{tuple_delimiter}"新国条"{tuple_delimiter}"EVENT"{tuple_delimiter}"《关于加快发展现代保险服务业的若干意见》的简称，旨在加快发展现代保险服务业，明确提出增加再保险市场主体和发展区域性再保险中心。"){record_delimiter}
("entity"{tuple_delimiter}"2013年"{tuple_delimiter}"DATE"{tuple_delimiter}"文本中提到的再保险保费数据的年份，反映了当时再保险行业的发展状况。"){record_delimiter}
("entity"{tuple_delimiter}"1238亿元"{tuple_delimiter}"CURRENCY"{tuple_delimiter}"2013年全国再保险保费的金额，以人民币计，显示了再保险市场的规模。"){record_delimiter}
("entity"{tuple_delimiter}"再保险市场"{tuple_delimiter}"FIN_TERM"{tuple_delimiter}"再保险交易的市场，正通过政策加快发展和创新，包括增加主体和提升风险分散能力。"){record_delimiter}
("entity"{tuple_delimiter}"中国"{tuple_delimiter}"LOCATION"{tuple_delimiter}"文本中提到的国家，再保险行业的发展所在地，政策支持以提升在全球市场的话语权。"){record_delimiter}
("entity"{tuple_delimiter}"海外再保险公司"{tuple_delimiter}"ORGANIZATION"{tuple_delimiter}"在中国开设分公司的外国再保险企业，共有7家，参与国内再保险市场。"){record_delimiter}
("relationship"{tuple_delimiter}"国务院"{tuple_delimiter}"新国条"{tuple_delimiter}"国务院下发《关于加快发展现代保险服务业的若干意见》政策文件，即新国条，以推动保险服务业和再保险市场发展。"){record_delimiter}
("relationship"{tuple_delimiter}"新国条"{tuple_delimiter}"再保险市场"{tuple_delimiter}"新国条明确提出加快发展再保险市场，增加市场主体和发展区域性中心，直接影响市场结构和创新。"){record_delimiter}
("relationship"{tuple_delimiter}"中再集团"{tuple_delimiter}"再保险市场"{tuple_delimiter}"中再集团是再保险市场中的唯一中资公司，体现了市场主体的集中性和重要性。"){record_delimiter}
("relationship"{tuple_delimiter}"2013年"{tuple_delimiter}"1238亿元"{tuple_delimiter}"2013年全国再保险保费为1238亿元，显示了该年份再保险市场的具体规模和财务数据。"){record_delimiter}
("relationship"{tuple_delimiter}"再保险市场"{tuple_delimiter}"中国"{tuple_delimiter}"再保险市场位于中国，并受到国家政策支持，以增强风险分散和全球市场影响力。"){record_delimiter}
("relationship"{tuple_delimiter}"海外再保险公司"{tuple_delimiter}"中国"{tuple_delimiter}"海外再保险公司在中国开设分公司，参与国内再保险市场，增加了市场多样性和竞争。"){record_delimiter}
("content_keywords"{tuple_delimiter}"再保险发展, 政策支持, 市场增长, 风险分散, 中国保险业"){completion_delimiter}

-示例 2-
文本：
################
广州农村商业银行网上银行网址http://www.961111.cn。
正常情况下，在插入 UKey（通过 USB 接口插入）后，系统会自动为您安装驱动程序。如系统未自动安装，请打开 UKey，选择“eP2Kauto.exe”进行安装，系统会自动安装，安装成功后，电脑的右下角会出现我行的 logo 。 广州农村商业银行下发的 UKey 已经安装有 CFCA 预制证书，客户初次使用 UKey，无需自行下载数字证书，直接登陆网上银行即可。 UKey 的初始密码为：88888888 ，首次登录时必须修改 UKey密码,建议您不要设置过于简单的密码。 个人客户遗忘 UKey 密码或 UKey 丢失，请到广州农村商业银行任一网点办理 UKey 换发业务
################
输出：
("entity"{tuple_delimiter}"广州农村商业银行"{tuple_delimiter}"ORGANIZATION"{tuple_delimiter}"一家银行机构，提供网上银行服务，负责UKey的发行、换发业务以及客户支持"){record_delimiter}
("entity"{tuple_delimiter}"网上银行"{tuple_delimiter}"FIN_PRODUCT"{tuple_delimiter}"广州农村商业银行提供的在线金融服务，通过网址http://www.961111.cn访问，使用UKey进行安全认证和登录"){record_delimiter}
("entity"{tuple_delimiter}"UKey"{tuple_delimiter}"FIN_PRODUCT"{tuple_delimiter}"USB接口的安全设备，用于网上银行登录，预装CFCA数字证书，初始密码为88888888，需首次修改，支持驱动程序自动安装"){record_delimiter}
("entity"{tuple_delimiter}"CFCA"{tuple_delimiter}"ORGANIZATION"{tuple_delimiter}"中国金融认证中心，提供数字认证服务，UKey设备预装其证书，确保网上银行交易安全"){record_delimiter}
("entity"{tuple_delimiter}"http://www.961111.cn"{tuple_delimiter}"FIN_TERM"{tuple_delimiter}"广州农村商业银行网上银行的官方网址，用于客户访问在线银行服务"){record_delimiter}
("relationship"{tuple_delimiter}"广州农村商业银行"{tuple_delimiter}"网上银行"{tuple_delimiter}"广州农村商业银行作为提供方，运营并管理网上银行服务，包括网址和UKey支持"){record_delimiter}
("relationship"{tuple_delimiter}"广州农村商业银行"{tuple_delimiter}"UKey"{tuple_delimiter}"广州农村商业银行下发UKey设备给客户，用于网上银行登录和安全认证"){record_delimiter}
("relationship"{tuple_delimiter}"网上银行"{tuple_delimiter}"UKey"{tuple_delimiter}"网上银行登录过程依赖UKey进行身份验证和安全操作，如密码修改"){record_delimiter}
("relationship"{tuple_delimiter}"UKey"{tuple_delimiter}"CFCA"{tuple_delimiter}"UKey预装有CFCA提供的数字证书，用于增强网上银行交易的安全性和可信度"){record_delimiter}
("relationship"{tuple_delimiter}"网上银行"{tuple_delimiter}"http://www.961111.cn"{tuple_delimiter}"网上银行通过特定网址http://www.961111.cn提供访问入口，客户可在此进行在线操作"){record_delimiter}
("content_keywords"{tuple_delimiter}"网上银行安全, UKey安装, 客户指南, 数字认证, 金融服务"){completion_delimiter}
...
RELATION_KEYWORDS: str = """你是一个NLP专家，擅长分析文本提取命名实体的关系和关键词。

-目标-
给定一段文本以及和这段文本相关的实体信息列表，从文本中识别这些实体之间所有的关系。
使用{language}作为输出语言。
...
KG_EXTRACTION_PROMPT: dict = {
    ...
    "FORMAT": {
        ...
        "record_delimiter": "##",
        "completion_delimiter": "<|COMPLETE|>",
        "entity_types": "PERSON, ORGANIZATION, LOCATION, FIN_PRODUCT, CURRENCY, DATE, OPERATION, FIN_TERM, EVENT",
        "language": "English",
    },
}

KG_EXTRACTION_PROMPT1: dict = {
    "English": {
        "TEMPLATE": TEMPLATE_EN,
        "CONTINUE": CONTINUE_EN,
        "IF_LOOP": IF_LOOP_EN,
